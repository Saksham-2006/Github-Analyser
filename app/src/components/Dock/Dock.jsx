...
function DockItem({ children, className = '', onClick, mouseX, spring, distance, magnification, baseItemSize, label }) {
  ...
  return (
    <motion.div
      ...
      className={`relative inline-flex items-center justify-center rounded-xl bg-[#0f0f0f] border-neutral-700 border shadow-md ${className}`}
      tabIndex={0}
      role="button"
      aria-haspopup="true"
      aria-label={label}
  ...
  return (
    <motion.div style={{ height, scrollbarWidth: 'none' }} className="mx-2 flex max-w-full items-center">
      <motion.div
        ...
        className={`${className} bg-[#0f0f0f] fixed bottom-2 left-1/2 transform -translate-x-1/2 flex items-end w-fit max-w-[calc(100vw-0.75rem)] gap-2 sm:gap-4 rounded-2xl border-neutral-700 border px-1 sm:px-1.5 py-1.5`}
